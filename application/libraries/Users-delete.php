...
class Users {
	...
	function isLoggedIn()
	{
		if( $this->ci->db_session->userdata('user') != NULL )
		{
	...
	function login( $username, $password )
	{
		if( $this->_validatePass($username,$password) )
		{
			...
			$this->ci->db_session->set_userdata('user',$userdata);
			return true;
		}
		else
		{
	...
	/**
	 * function register( $username, $password, $email, $emailcheck = true )include 'Users.php';

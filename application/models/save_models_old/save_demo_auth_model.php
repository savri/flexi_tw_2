...
class Demo_auth_model extends CI_Model {
	...
	function register_account()
	{
		...
		$validation_rules = array(
			array('field' => 'register_first_name', 'label' => 'First Name', 'rules' => 'trim|required|xss_clean'),
			array('field' => 'register_last_name', 'label' => 'Last Name', 'rules' => 'trim|required|xss_clean'),
			array('field' => 'register_phone_number', 'label' => 'Phone Number', 'rules' => 'numeric|required'),
		...
		if ($this->form_validation->run())
		{
			...
			if ($response)
			{
				...
				if ($instant_activate && $this->flexi_auth->login($email, $password))
				{
					...
					$this->firephp->log("Auto activation - Logging user in");
